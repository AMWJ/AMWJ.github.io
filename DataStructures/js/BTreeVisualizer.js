(function () {
    ...
    redisplayBTree = function (elements, edges) {
        ...
        var rowHeight = function (depth) {
            return Math.sqrt(blockSize) * 20 * depth;
        };
        for (var i = 0; i < elements.length; i++) {
            if (lastElements[elements[i].depth] == null || lastElements[elements[i].depth].elementOnRow < elements[i].elementOnRow) {
                lastElements[elements[i].depth] = elements[i];
            }
        ...
        for (var i = 0; i < Object.keys(lastElements).length; i++) {
            rowWidths[i] = blockSize + lastElements[i].elementOnRow * blockSize + lastElements[i].indexOnRow * blockSize / 3;
        }
        var scalingFactor = 1;
        var maxRowWidth = rowWidths.reduce(function (a, b) { return Math.max(a, b) });
        ...
        */
        ...
        var rects = newGs.append("rect")
            .attr("x", function (d, i) { return rowStartingPoints[d.depth] + blockSize * d.elementOnRow + blockSize / 3 * d.indexOnRow; })
            .attr("y", function (d, i) { return rowHeight(d.depth); })
            .attr("width", blockSize)
            .attr("height", blockSize)
            .attr("fill", function (d, i) {
                return colorFn(d.id) || "#FF7777";
            })
            .attr("stroke", "black")
        var labels = newGs.append("text")
            .attr("x", function (d, i) { return rowStartingPoints[d.depth] + blockSize * d.elementOnRow + blockSize / 3 * d.indexOnRow + blockSize / 2; })
            .attr("y", function (d, i) { return rowHeight(d.depth) + blockSize / 2; })
            .attr("dy", ".35em")
        ...
        */
        ...
        newEdges.attr("opacity", 1)
            ...
            .attr("x1", function (d, i) { return rowStartingPoints[d.depth - 1] + blockSize * (d.from.elementOnRow + 1) + blockSize / 3 * d.from.nodeOnRow; })
            .attr("y1", function (d, i) { return rowHeight(d.depth - 1) + blockSize; })
            .attr("x2", function (d, i) { return rowStartingPoints[d.depth - 1] + blockSize * (d.from.elementOnRow + 1) + blockSize / 3 * d.from.nodeOnRow; })
            .attr("y2", function (d, i) { return rowHeight(d.depth - 1); })
            .transition().duration(phase)
            .attr("x2", function (d, i) { return rowStartingPoints[d.depth] + blockSize * (d.to.firstElement + d.to.lastElement) / 2 + blockSize / 3 * d.to.node + blockSize / 2; })
